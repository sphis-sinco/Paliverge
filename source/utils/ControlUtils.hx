...
class ControlsSave
{
	...
	public function save(path:Null<String> = null)
	{
		...
		for (grp in controlGroups)
		{
			...
			for (key in keys)
			{
				if (ControlUtils.controls.exists(grp + key))
				{
					...
					for (controlKey in ControlUtils.controls.get(grp + key))
					{
		...
		sys.io.File.saveContent(path, '<!DOCTYPE paliverge-controls-xml>\n' + xml.toString());
		#else
	...
	public function load(path:Null<String> = null)
	{
		...
		var xml:Access = new Access(Xml.parse(PathUtils.getText(path)).firstElement());
		...
		ControlUtils.controls = controls_map;
